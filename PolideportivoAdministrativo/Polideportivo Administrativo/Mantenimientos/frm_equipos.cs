...
namespace Polideportivo_Administrativo.Mantenimientos
{
    public partial class frm_equipos : Form
    {
        ...
        void llenarGridEquipos()
        {
            ...
            {
                ds = new DataSet();
                dat = new OdbcDataAdapter("SELECT PK_idEquipo AS Codigo, nombre_equipo AS Nombre, descripcion_equipo AS Descripcion FROM tbl_equipos WHERE estado_equipo=1", nueva.conectar());
                dat.Fill(ds);
        ...
        private void Btn_buscar_Click(object sender, EventArgs e)
        {
            ...
            {
                ds = new DataSet();
                dat = new OdbcDataAdapter("SELECT PK_idEquipo AS Codigo, nombre_equipo AS Nombre, descripcion_equipo AS Descripcion FROM tbl_equipos WHERE PK_idEquipo='"+Txt_buscar.Text+"' OR nombre_equipo = '"+Txt_buscar.Text+"'", nueva.conectar());
                dat.Fill(ds);
                Dgv_equipos.DataSource = ds.Tables[0];
            }
            catch (OdbcException er)
