        ...
        private void Btn_guardar_Click(object sender, EventArgs e)
        {
            ...
            if (bBoton_ingreso == true)
            {
                ...
                if (bIngresoCorrecto)
                {
                    ...
                    cmd = new OdbcCommand("INSERT INTO tbl_bitacora(PK_idUsuario, accion, fecha, hora, tabla, host) VALUES (1,'INSERTAR','" + sFecha + "','" + sHora + "', 'tbl_sanciones', '" + host + "')", conexion.conectar());
                    cmd.ExecuteNonQuery();
                }
            }
            else if (bBoton_modificar == true)
            {
                ...
                if (bIngresoCorrecto)
                {
                    ...
                    habilitarTodo();
                    cmd = new OdbcCommand("INSERT INTO tbl_bitacora(PK_idUsuario, accion, fecha, hora, tabla, host) VALUES (1,'MODIFICAR','" + sFecha + "','" + sHora + "', 'tbl_sanciones', '" + host + "')", conexion.conectar());
                    cmd.ExecuteNonQuery();
                }
            }
            else if (bBoton_eliminar == true)
            {
               // bool ingresoCorrecto = true;
                ...
                if (bIngresoCorrecto)
                {
                    ...
                    cmd = new OdbcCommand("INSERT INTO tbl_bitacora(PK_idUsuario, accion, fecha, hora, tabla, host) VALUES (1,'ELIMINAR','" + sFecha + "','" + sHora + "', 'tbl_sanciones', '" + host + "')", conexion.conectar());
                    cmd.ExecuteNonQuery();
                }
            }
        }
