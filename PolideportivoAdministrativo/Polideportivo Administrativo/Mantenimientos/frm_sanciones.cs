...
namespace Polideportivo_Administrativo.Mantenimientos
{
    public partial class frm_sanciones : Form
    {
        ...
        void llenarGridSanciones()
        {
            ...
            {
                ds = new DataSet();
                dat = new OdbcDataAdapter("SELECT PK_idSancion AS Codigo, nombre_sancion AS Nombre, descripcion_sancion AS Descripcion FROM tbl_sanciones WHERE estado_sancion=1", conexion.conectar());
                dat.Fill(ds);
        ...
        private void Btn_buscar_Click(object sender, EventArgs e)
        {
            ...
            {
                ds = new DataSet();
                dat = new OdbcDataAdapter("SELECT PK_idSancion AS Codigo, nombre_sancion AS Nombre, descripcion_sancion AS Descripcion FROM tbl_sanciones WHERE PK_idSancion='" + Txt_buscar.Text + "' OR nombre_sancion = '" + Txt_buscar.Text + "'", conexion.conectar());
                dat.Fill(ds);
                Dgv_sanciones.DataSource = ds.Tables[0];
            }
            catch (OdbcException er)
