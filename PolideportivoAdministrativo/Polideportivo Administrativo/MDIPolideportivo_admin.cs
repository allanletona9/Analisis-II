...
//Autor Diego Gomez
        ...
        bool ventanaGoleadores = false;
        frm_goleadores mostrarGoleadores = new frm_goleadores();

        private void visualizacionGoleadoresToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Form frmC = Application.OpenForms.Cast<Form>().FirstOrDefault(x => x is frm_jugadores);
            if (ventanaGoleadores == false || frmC == null)
            {
                if (frmC == null)
                {
                    mostrarGoleadores = new frm_goleadores();
                }

                mostrarGoleadores.MdiParent = this;
                mostrarGoleadores.Show();
                Application.DoEvents();
                ventanaGoleadores = true;
            }
            else
            {
                mostrarGoleadores.WindowState = System.Windows.Forms.FormWindowState.Normal;
            }
        }
