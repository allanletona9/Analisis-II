...
namespace Polideportivo_Administrativo
{
    public partial class MDIPolideportivo_admin : Form
    {
        ...
        public MDIPolideportivo_admin(String nombreUsuario, String tipo)
        {
            ...
            this.tipo = tipo;
            InitializeComponent();
            bloquearBotones(tipo);
            lblUsuario.Text = "Usuario: " + nombreUsuario;

        }

        private void bloquearBotones(String tipo)
        {
            if (tipo == "1")
            {
        ...
        conexion nueva = new conexion();
            ...
            cmd = new OdbcCommand("INSERT INTO tbl_bitacora (PK_idBitacora," +
                ...
                "tabla," +
                "host) " +
                "VALUES('' ,(SELECT PK_idUsuario FROM tbl_usuarios WHERE Pk_idUsuario = '" + nombreUsuario + "')" +
...
//Autor Diego Gomez
        ...
        bool ventanaControlGoleadores = false;
        frm_goleadores mostrarControlGoleadores = new frm_goleadores();

        private void controlDeGoleadoresToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Form frmC = Application.OpenForms.Cast<Form>().FirstOrDefault(x => x is frm_jugadores);
            if (ventanaControlGoleadores == false || frmC == null)
            {
                if (frmC == null)
                {
                    mostrarControlGoleadores = new frm_goleadores();
                }

                mostrarControlGoleadores.MdiParent = this;
                mostrarControlGoleadores.Show();
                Application.DoEvents();
                ventanaControlGoleadores = true;
            }
            else
            {
                mostrarControlGoleadores.WindowState = System.Windows.Forms.FormWindowState.Normal;
            }
        ...
        {
            new reporte_jugadores().Show();
        }

        private void campeonatosToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            new reporte_campeonato().Show();
        }

        private void equipoToolStripMenuItem_Click(object sender, EventArgs e)
        {
            new reporte_equipos().Show();
        }

        private void equiposPorCampeonatoToolStripMenuItem_Click(object sender, EventArgs e)
        {
            new reporte_empleados().Show();
        }
