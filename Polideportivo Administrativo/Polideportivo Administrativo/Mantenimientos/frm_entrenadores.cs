...
namespace Polideportivo_Administrativo.Mantenimientos
{
    public partial class frm_entrenadores : Form
    {
        ...
        void llenarGridEntrenadores()
        {
            ...
            {
                ds = new DataSet();
                dat = new OdbcDataAdapter("SELECT PK_idEntrenador AS Codigo, nombre_entrenador AS Nombre, apellido_entrenador as Apellido, especialidad_entrenador as Especialidad FROM tbl_entrenadores WHERE estado_entrenador=1", conexion.conectar());
                dat.Fill(ds);
        ...
        private void Btn_buscar_Click(object sender, EventArgs e)
        {
            ...
            {
                ds = new DataSet();
                dat = new OdbcDataAdapter("SELECT PK_idEntrenador AS Codigo, nombre_entrenador AS Nombre, apellido_entrenador as Apellido, especialidad_entrenador as Especialidad FROM tbl_entrenadores WHERE PK_idEntrenador='" + Txt_buscar.Text + "' OR nombre_entrenadors = '" + Txt_buscar.Text + "'", conexion.conectar());
                dat.Fill(ds);
                Dgv_entrenadores.DataSource = ds.Tables[0];
            }
            catch (OdbcException er)
