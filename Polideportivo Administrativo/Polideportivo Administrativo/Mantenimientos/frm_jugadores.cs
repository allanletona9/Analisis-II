...
namespace Polideportivo_Administrativo.Mantenimientos
{
    public partial class frm_jugadores : Form
    {
        ...
        void llenarGridJugadores()
        {
            ...
            {
                ds = new DataSet();
                dat = new OdbcDataAdapter("SELECT PK_idJugador AS Codigo, nombre_jugador AS Nombre, apellido_jugador as Apellido, fecha_nacimiento_jugador as FechaDeNacimiento, numeroCamisola_jugador as Camisola, telefono_jugador as Telefono, dpi_jugador as DPI FROM tbl_jugadores WHERE estado_jugador=1", conexion.conectar());
                dat.Fill(ds);
        ...
        private void Btn_buscar_Click(object sender, EventArgs e)
        {
            ...
            {
                ds = new DataSet();
                dat = new OdbcDataAdapter("PK_idJugador AS Codigo, nombre_jugador AS Nombre, apellido_jugador as Apellido, fecha_nacimiento_jugador as FechaDeNacimiento, numeroCamisola_jugador as Camisola, telefono_jugador as Telefono, dpi_jugador as DPI  FROM tbl_jugadores WHERE PK_idJugador='" + Txt_buscar.Text + "' OR nombre_jugador = '" + Txt_buscar.Text + "'", conexion.conectar());
                dat.Fill(ds);
                Dgv_jugadores.DataSource = ds.Tables[0];
            }
            catch (OdbcException er)
