        ...
        private void Btn_guardar_Click(object sender, EventArgs e)
        {
            ...
            if (boton_ingreso == true)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        cmd = new OdbcCommand("INSERT INTO tbl_arbitros(nombre_arbitro, apellido_arbitro, estado_arbitro) VALUES ('"
                            + Txt_nombreArbitro.Text + "', '" + Txt_apellidoArbitro.Text + "' , 1)", conexion.conectar());
                        cmd.ExecuteNonQuery();
                    }
                }
                ...
                if (ingresoCorrecto)
                {
                    ...
                    int iLastId = 0;
                    OdbcCommand sql = new OdbcCommand("SELECT PK_idArbitro FROM tbl_arbitros ORDER by PK_idArbitro DESC LIMIT 1", conexion.conectar());
                    OdbcDataReader almacena = sql.ExecuteReader();
                    while (almacena.Read() == true)
                    {
                    ...
                    cmd = new OdbcCommand("INSERT INTO tbl_arbitros_deportes(PK_idArbitro, PK_idDeporte) VALUES ("
                            + iLastId.ToString() + ", " + lIdDeporte[Cbo_deporte.SelectedIndex].ToString() + " )", conexion.conectar());
                    cmd.ExecuteNonQuery();

                    cmd = new OdbcCommand("INSERT INTO tbl_bitacora(PK_idUsuario, accion, fecha, hora, tabla, host) VALUES (1,'INSERTAR','" + sFecha + "','" + sHora + "', 'tbl_arbitros', '" + host + "')", conexion.conectar());
                    cmd.ExecuteNonQuery();
            ...
            else if (boton_modificar == true)
            {
                ...
                try
                {
                    ...
                    else
                    {
                        if (Rdb_habilitado.Checked == true)
                        {
                          ...
                          " WHERE PK_idArbitro =" + Txt_codigoArbitro.Text, conexion.conectar());
                            cmd.ExecuteNonQuery();
                        }
                ...
                if (ingresoCorrecto)
                {
                    ...
                    cmd = new OdbcCommand("INSERT INTO tbl_bitacora(PK_idUsuario, accion, fecha, hora, tabla, host) VALUES (1,'MODIFICAR','" + sFecha + "','" + sHora + "', 'tbl_arbitros', '" + host + "')", conexion.conectar());
                    cmd.ExecuteNonQuery();
                }
            }
            else if (boton_eliminar == true)
            {
                ...
                try
                {
                   ...
                   + Txt_codigoArbitro.Text, conexion.conectar());
                    cmd.ExecuteNonQuery();
                }
                catch (OdbcException ex)
                {
                ...
                if (ingresoCorrecto)
                {
                    ...
                    cmd = new OdbcCommand("INSERT INTO tbl_bitacora(PK_idUsuario, accion, fecha, hora, tabla, host) VALUES (1,'ELIMINAR','" + sFecha + "','" + sHora + "', 'tbl_arbitros', '" + host + "')", conexion.conectar());
                    cmd.ExecuteNonQuery();
                }
            }
        }
